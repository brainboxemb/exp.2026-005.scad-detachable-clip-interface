...
// Keep the complete top guide inside the 10 mm receiver footprint.
// The central region carries the full X/Z insertion lead-in; each 2 mm end
// region tapers that lead-in back to the ordinary receiver width.
NODE_RECEIVER_TOP_GUIDE_TRANSITION_LENGTH = 2.0;
NODE_RECEIVER_TOP_GUIDE_ACTIVE_LENGTH =
    NODE_RECEIVER_FUNCTIONAL_LENGTH - 2 * NODE_RECEIVER_TOP_GUIDE_TRANSITION_LENGTH; // 6 mm
...
assert(abs(NODE_RECEIVER_TOP_GUIDE_ACTIVE_LENGTH - 6.0) < 0.0001);
...
// the full X/Z lead-in active for the central 6 mm on +/-X and tapers it back
// to the ordinary top face over 2 mm per Y end, all inside the 10 mm footprint.
// There is no separate +/-Y top chamfer
...
module _node_positive_x_top_guide_cut() {
    ...
    //
    //   Y=-5      Y=-3         Y=+3      Y=+5
    //    point -> full section ======> point
    //
    // The central 6 mm carries the complete X/Z lead-in, while both 2 mm end
    // regions taper into the ordinary outer/top corner within the 10 mm part.
    // Critically, there are NO polyhedron faces at Y=+/-3, so the exported
