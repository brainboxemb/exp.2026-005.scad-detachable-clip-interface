...
// Keep one simple X/Z insertion chamfer over the complete 10 mm receiver
// length. There is deliberately no separate Y taper: the snap is open at both
// Y ends, so the guide only needs to narrow inward on +/-X.
...
// The top guide is deliberately TWO-SIDED and simpler: one constant X/Z
// chamfer is extruded over the complete 10 mm receiver length. There is no
// separate +/-Y top chamfer or Y taper because the removable snap is open at
// those ends. Opposite X sides are generated by mirror(), guaranteeing
// symmetry.
...
module _node_positive_x_top_guide_cut() {
    // Constant X/Z triangular chamfer over the full 10 mm Y length.
    // At Z=3.6 the receiver remains 10.0 mm wide; at Z=4.0 it narrows to
    // 9.2 mm. Extending that same section through Y avoids hourglass plan
    // geometry, V-shaped side profiles and thin end fins.
    rotate([90, 0, 0])
        linear_extrude(
