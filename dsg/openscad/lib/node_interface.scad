...
// Keep the insertion chamfer local within the 10 mm receiver footprint.
// The full X/Z chamfer is active for 8 mm. Over the final 1 mm at each Y end
// only the chamfer DEPTH tapers to zero; the top plane stays at the same Z.
NODE_RECEIVER_TOP_GUIDE_TRANSITION_LENGTH = 1.0;
NODE_RECEIVER_TOP_GUIDE_ACTIVE_LENGTH =
    NODE_RECEIVER_FUNCTIONAL_LENGTH - 2 * NODE_RECEIVER_TOP_GUIDE_TRANSITION_LENGTH; // 8 mm
NODE_RECEIVER_TOP_GUIDE_LENGTH = NODE_RECEIVER_FUNCTIONAL_LENGTH; // 10 mm

// Tessellation only; this does not introduce another physical dimension.
// A smoothstep depth fade is sampled densely enough that the 1 mm local
// transition does not become one visible straight cut plane.
NODE_RECEIVER_TRANSITION_STEPS = 12;
NODE_RECEIVER_TRANSITION_SECTION_THICKNESS = 0.02;

// The standalone receiver and plate support use the same compact footprint as
// the snap interface. No extra Y support is added just to carry the guide.
...
assert(abs(NODE_RECEIVER_TOP_GUIDE_TRANSITION_LENGTH - 1.0) < 0.0001);
assert(abs(NODE_RECEIVER_TOP_GUIDE_ACTIVE_LENGTH - 8.0) < 0.0001);
...
function node_receiver_top_guide_active_length() = NODE_RECEIVER_TOP_GUIDE_ACTIVE_LENGTH;
function node_receiver_top_guide_transition_length() = NODE_RECEIVER_TOP_GUIDE_TRANSITION_LENGTH;
...
// The top guide follows the same Y envelope: full X/Z chamfer for 8 mm, then
// 1 mm per end where only the radial cut depth returns to zero. The top Z plane
// does not slope down in Y. There is no separate +/-Y capture chamfer because
// the removable snap is open at those ends. Opposite X sides are generated by
// mirror(), guaranteeing symmetry.

function _node_smoothstep01(t) = t * t * (3 - 2 * t);
function _node_receiver_depth_factor(t) =
    max(0.001, 1 - _node_smoothstep01(t));
...
module _node_positive_x_top_guide_active_cut() {
    rotate([90, 0, 0])
        linear_extrude(
            height = NODE_RECEIVER_TOP_GUIDE_ACTIVE_LENGTH,
            center = true,
            convexity = 10
        )
            polygon(points = [
...
module _node_positive_x_top_guide_transition_section(y, factor) {
    xo = NODE_RECEIVER_WIDTH / 2 + 0.01;
    xi_full = NODE_RECEIVER_TOP_WIDTH / 2;
    xi = xo - (xo - xi_full) * factor;

    translate([0, y, 0])
        rotate([90, 0, 0])
            linear_extrude(
                height = NODE_RECEIVER_TRANSITION_SECTION_THICKNESS,
                center = true,
                convexity = 10
            )
                polygon(points = [
                    [xi, NODE_RECEIVER_HEIGHT + 0.01],
                    [xo, NODE_RECEIVER_HEIGHT + 0.01],
                    [xo, NODE_RECEIVER_CAPTURE_TOP_Z]
                ]);
}

module _node_positive_y_top_guide_transition() {
    ya = NODE_RECEIVER_TOP_GUIDE_ACTIVE_LENGTH / 2;
    length = NODE_RECEIVER_TOP_GUIDE_TRANSITION_LENGTH;
    steps = NODE_RECEIVER_TRANSITION_STEPS;

    // Blend only radial cut depth. smoothstep has zero slope at t=0 and t=1,
    // so the local guide leaves the 8 mm active zone without a hard straight
    // crease and reaches the untouched 10 mm end without another abrupt face.
    for (i = [0 : steps - 1]) {
        t0 = i / steps;
        t1 = (i + 1) / steps;

        hull() {
            _node_positive_x_top_guide_transition_section(
                ya + length * t0,
                _node_receiver_depth_factor(t0)
            );
            _node_positive_x_top_guide_transition_section(
                ya + length * t1,
                _node_receiver_depth_factor(t1)
            );
        }
    }
}
...
module _node_positive_x_lower_cut_transition_section(y, factor) {
    xo = NODE_RECEIVER_WIDTH / 2 + 0.01;
    xi_full = NODE_RECEIVER_LOWER_WIDTH / 2;
    xi = xo - (xo - xi_full) * factor;
        ...
        rotate([90, 0, 0])
            linear_extrude(
                ...
                polygon(points = [
                    [xi, 0],
                    [xo, 0],
                    [xo, NODE_RECEIVER_RAMP_TOP_Z],
                    [xi, NODE_RECEIVER_LOWER_Z]
                ]);
}

// Positive-Y lower cut blend.
// The source-derived X/Z section remains unchanged in the central 8 mm.
// Only its radial cut depth fades over the final 1 mm.
module _node_positive_y_lower_cut_transition() {
    ya = NODE_RECEIVER_LOWER_ACTIVE_LENGTH / 2;
    length = NODE_RECEIVER_LOWER_TRANSITION_LENGTH;
    steps = NODE_RECEIVER_TRANSITION_STEPS;
    ...
    for (i = [0 : steps - 1]) {
        ...
        hull() {
            _node_positive_x_lower_cut_transition_section(
                ya + length * t0,
                _node_receiver_depth_factor(t0)
            );
            _node_positive_x_lower_cut_transition_section(
